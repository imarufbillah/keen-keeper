      ...
      <div className="flex flex-col gap-2 justify-center items-center w-full">
        ...
        <div className="mt-0.5 flex gap-2">
          {tags.map((tag, index) => (
            <span
              key={index}
              className="px-2.5 py-1 rounded-full text-xs font-medium bg-[#CBFADB] text-primary"
            >
              {tag}
            </span>
          ))}
        ...
        <div className="">
          <span
            className={`px-2.5 py-1 rounded-full text-xs font-medium ${status === "overdue" ? "bg-[#FEE2E2] text-red-600" : status === "almost due" ? "bg-[#FEF3C7] text-amber-700" : "bg-primary text-white"} bg-[#CBFADB] text-primary`}
          >
            {status}
          </span>
        </div>
